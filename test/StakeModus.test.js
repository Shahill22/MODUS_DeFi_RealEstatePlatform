  ...
  function ([owner, account1, account2, account3]) {
    describe("1. Before deployment", async function () {
      ...
      it("1.1. should fail when trying to deploy with wrong argument types", async function () {
        await expectInvalidArgument.address(
          this.stakingContract.initialize(
            this.token.address,
    ...
    describe("2. On deployment", async function () {
      ...
      it("2.2. should set the right rewardsAddress", async function () {
        expect(await this.stakingContract.rewardsAddress()).to.equal(
          rewardsAddress
        );
      });
      ...
      it("2.4 deployer has the default admin role", async function () {
        expect(
          await this.stakingContract.getRoleMemberCount(DEFAULT_ADMIN_ROLE)
        ).to.be.bignumber.equal("1");
        expect(
          await this.stakingContract.getRoleMember(DEFAULT_ADMIN_ROLE, 0)
        ).to.equal(owner);
      });

      it("2.5 deployer has the pauser role", async function () {
        expect(
          await this.stakingContract.getRoleMemberCount(PAUSER_ROLE)
        ).to.be.bignumber.equal("1");
        expect(
          await this.stakingContract.getRoleMember(PAUSER_ROLE, 0)
        ).to.equal(owner);
      });
      ...
      it("2.7 deployer has the rewards distribution role", async function () {
        expect(
          await this.stakingContract.getRoleMemberCount(
            REWARDS_DISTRIBUTOR_ROLE
          )
        ).to.be.bignumber.equal("1");
        expect(
          await this.stakingContract.getRoleMember(REWARDS_DISTRIBUTOR_ROLE, 0)
        ).to.equal(owner);
      });

      it("2.8 deployer has the owner role", async function () {
        expect(
          await this.stakingContract.getRoleMemberCount(OWNER_ROLE)
        ).to.be.bignumber.equal("1");
        expect(
          await this.stakingContract.getRoleMember(OWNER_ROLE, 0)
        ).to.equal(owner);
      });
    });
    ...
    describe("3. Setup", async function () {
      ...
      it("3.3. setRewardAddress: should revert when contract is not paused", async function () {
        ...
        await expectRevert(
          this.stakingContract.setRewardAddress(rewardsAddress, from(owner)),
          revertMessage
        );
      });
      ...
      it("3.6. setRewardAddress: should set reward address correctly", async function () {
        ...
        await this.stakingContract.pause();
        await this.stakingContract.setRewardAddress(
          newRewardsAddress,
          from(owner)
        );
        await this.stakingContract.unpause();
    ...
    describe("4. Deposit and withdraw", async function () {
      ...
      it("4.2. deposit: should revert when contract is paused", async function () {
        ...
        await this.stakingContract.pause();
        await expectRevert(
          this.stakingContract.deposit(depositAmount),
          revertMessage
        );
        await this.stakingContract.unpause();
      });
